...
{
    string pomocniczy;
    getline (plik, pomocniczy); //int potem
...
void odwrtonySortInt (string plik1, string pomocniczy1, string pomocniczy2)
{
    ...
    while (true)
    {
        ...
        if (pomoc1 >> dane1.liczba) //int ktorego nie ma w  funkcji
        {
...
void liczbaWystapien (string plik1, string pomocniczy1, string pomocniczy2, int& licznikMax, int& maxInt)
{
    ...
    while ( pierwszy >> dane1.liczba)
    {
        ...
        pierwszy.close();
        pierwszy.open(plik1.c_str(), fstream :: out); //zeruje pierwszy; pomoc1 do pierwszego
...
void SymmetricDifference (string plik1, string plik2, string pomocniczy)
{
    ...
    pomoc.open (pomocniczy.c_str(), fstream::out); //zeruje pomocniczy
    pomoc.close();
    ...
    string linia;
    while (getline(drugi, linia)) { //kopiuje drugi do pomocniczego
        pomoc << linia << endl;
    }
    ...
    drugi.open(plik2.c_str(), fstream::out);  //zeruje drugi
    drugi.close();

    pomoc.open(pomocniczy.c_str(), fstream::in);


    while (pomoc >> dane1.liczba)    //lece po pomoc; brakujacy int
    {
        ladowanieDanych(dane1, pomoc);
        pierwszy.open (plik1.c_str(), fstream::in);

        while (pierwszy >> dane2.liczba) //analogicznie
        {
            ladowanieDanych(dane2, pierwszy);
        ...
        if (sprawdzenie == false)
        {
            ...
            doPliku(dane1, pierwszy); //jesli tak, dodaje do pierwszego pliku
            pierwszy.close();
        }
        else
        {
            drugi.open(plik2.c_str(), fstream::app | fstream::out);
            doPliku(dane1, drugi); //jesli nie, to dodaje do drugiego
            drugi.close();
        }
        sprawdzenie = false;
    }
